---
{
  "title": "Uczyń czas widzialnym (alarmy + kolory w kalendarzu)",
  "summary": "ADHD wiąże się ze ślepotą czasową — czas nie jest odczuwany intuicyjnie. Wynieś go na zewnątrz konkretnymi wskazówkami wzrokowymi: koloruj wpisy w kalendarzu, ustaw zewnętrzne markery. Wpis w złym kolorze potrafi sprawić, że spóźnisz się pół godziny.",
  "icon": "mdi:tools",
  "order": 64,
  "conditions": [
    "adhd",
    "audhd"
  ],
  "addresses": [
    {
      "target": "chaos-czas-organizacja",
      "kind": "symptom",
      "evidence": "C",
      "community": "średni",
      "note": "Kolorowanie kalendarza daje wizualny obraz całego dnia, którego mózg ze ślepotą czasu nie wytwarza sam."
    },
    {
      "target": "slepota-czasu",
      "kind": "mechanism",
      "evidence": "C",
      "community": "średni",
      "note": "Eksternalizuje percepcję czasu — robi widzialnym to, czego mózg nie czuje intuicyjnie."
    }
  ],
  "resources": [
    {
      "title": "I Spent Years Pretending I Was Fine (uczyń czas widzialnym)",
      "type": "video",
      "url": "https://www.youtube.com/watch?v=hPSp2vqAjWo&t=1494s",
      "note": "„Musiałam ciężko pracować, by uczynić czas widzialnym — dziś spóźniłam się pół godziny, bo wpis był w złym kolorze.”"
    },
    {
      "title": "Martine McCutcheon: budzik i alarmy na wszystko",
      "type": "video",
      "url": "https://www.youtube.com/watch?v=mTT4Qrfrm-A&t=4846s",
      "author": "Martine McCutcheon",
      "note": "Bez dzwonka/alarmu nie wraca do tego, co miała zrobić — od mycia zębów po wyprawienie syna."
    },
    {
      "title": "My Secret To Keeping a Neurodiverse Family FUNCTIONAL — lista tam, gdzie ją widzisz",
      "type": "video",
      "url": "https://www.youtube.com/watch?v=PJLI9rcnfg8&t=2173s",
      "author": "Hester Grainger",
      "note": "Lista w zamkniętym notesie/telefonie nie działa — wynieś ją na białe tablice po domu albo klips; z oczu = z myśli."
    },
    {
      "title": "Secrets to Manage ADHD Shame — Dr Ayan Panja (alarmy, timery i listy realnie pomagają)",
      "type": "video",
      "url": "https://www.youtube.com/watch?v=Tpw_C_kovJU&t=2449s",
      "author": "Dr Ayan Panja",
      "note": "„Alarmy i timery potrafią zmienić trajektorię kogoś, kto walczy z rutyną… wbrew influencerom: listy są świetne przy ADHD, bo można je odhaczać.” Odhaczanie daje nagrodę, której mózg ADHD inaczej nie rejestruje."
    },
    {
      "title": "10 Rules That Traumatise ADHD & Autistic Brains (przeliczaj minuty na sekundy + minutnik z piaskiem)",
      "type": "video",
      "url": "https://www.youtube.com/watch?v=Al5cYCOv4UM&t=2495s",
      "author": "ADHD Chatter",
      "note": "„Zamiast »5 minut« powiedz »300 sekund« — widzisz, jak 300 zaczyna odliczać, i nagle to wcale nie jest długo.” Wspomagaj wizualnymi minutnikami z piaskiem; widoczny upływ czasu motywuje."
    }
  ],
  "lang": "pl",
  "studies": [
    {
      "title": "Patterns of time processing and time management in children with disabilities: effectiveness of an intervention (time aids)",
      "type": "rct",
      "url": "https://pubmed.ncbi.nlm.nih.gov/24417452/",
      "year": 2014,
      "finding": "Mały RCT (n=37, wiek 6-11, ADHD oraz ASD/niepełnosprawność intelektualna/rozszczep kręgosłupa/MPD) interwencji z pomocami czasowymi, które uzewnętrzniają i uwidaczniają czas. Grupa interwencyjna poprawiła zdolność przetwarzania czasu (d=0,81, duży efekt) i zarządzania czasem (d=0,68, średni efekt) istotnie bardziej niż grupa z listy oczekujących. Autorzy nazywają to 'wstępnym dowodem'. Mieszana populacja i mała próba ograniczają wnioski swoiste dla ADHD."
    },
    {
      "title": "The Efficacy of Visual Activity Schedule Intervention in Reducing Problem Behaviors in Children With ADHD Between the Age of 5 and 12 Years: A Systematic Review",
      "type": "review",
      "url": "https://pubmed.ncbi.nlm.nih.gov/35035237/",
      "year": 2022,
      "finding": "Przegląd systematyczny wizualnych planów aktywności (wizualnej reprezentacji dnia) u dzieci z ADHD. Tylko 4 badania spełniły kryteria; włączony RCT oceniono jako niskiej jakości. Interwencje wiązały się z redukcją problemowych zachowań i większą satysfakcją, ale baza dowodowa jest cienka i nie ukierunkowana swoiście na percepcję czasu."
    },
    {
      "title": "Time Perception in Adult ADHD: Findings from a Decade — A Review",
      "type": "review",
      "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC9962130/",
      "year": 2023,
      "finding": "Przegląd narracyjny potwierdza, że zaburzenie percepcji czasu / ślepota czasowa to spójna cecha ADHD (silny mechanizm). Wprost zaznacza, że podręczniki leczenia dorosłego ADHD nie uwzględniają deficytu percepcji czasu, a dowodów interwencyjnych jest minimalnie — praktycznie brak kontrolowanej oceny zewnętrznych/wizualnych pomocy czasowych czy kolorowania kalendarza."
    }
  ]
}
---

Przy ślepocie czasu mózg nie postrzega upływu czasu intuicyjnie, więc zarządzanie nim trzeba przenieść na zewnątrz, robiąc czas WIDZIALNYM. Najprostsza dźwignia: kolory w kalendarzu. Każdy typ wydarzenia ma swój kolor, dzięki czemu rzut oka na dzień od razu mówi, co i kiedy — a brak lub zły kolor potrafi realnie kosztować pół godziny spóźnienia. Warto dokładać inne zewnętrzne markery (alarmy, widoczne zegary, tablica), żeby żaden termin nie był „niewidzialny". To różni się od alarmów-jako-czasu: tu chodzi o WIZUALNĄ reprezentację całego dnia naraz, nie o pojedyncze pingi.
